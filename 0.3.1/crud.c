...
void eliminarRestaurantePorId(){
    ...
    while(fread(&reg, sizeof(Restaurante), 1, archivo) == 1){
        if(reg.id_restaurante == id_a_eliminar){
            ...
            eliminarUbicacionRestaurante(id_a_eliminar);
        }
        else{
...
void eliminarUbicacionRestaurante(int id_ubicacion){
    ...
    auxiliar = fopen("auxiliar_restaurante.dat", "wb");
    ...
    while(fread(&ubicacion, sizeof(UbicacionRestaurante), 1, archivo) == 1){
        if(ubicacion.id_ubicacion != id_ubicacion){ fwrite(&ubicacion, sizeof(UbicacionRestaurante), 1, auxiliar); }
    }

    fclose(archivo);
    fclose(auxiliar);

    remove("ubicacion_restaurante.dat");
    rename("auxiliar_restaurante.dat", "ubicacion_restaurante.dat");
}
// =========================
...
void actualizarRestauranteXid(Restaurante restaurante){
    ...
    archivo = fopen("restaurante.dat", "r+b");

    if(archivo == NULL){
        printf("\nError al abrir restaurante.dat");
        return;
    }

    while(fread(&restaurante, sizeof(Restaurante), 1, archivo) == 1){
        if(restaurante.id_restaurante == idBuscado){
            encontrado = 1;
            ...
            fseek(archivo, -sizeof(Restaurante), SEEK_CUR);
            fwrite(&restaurante, sizeof(Restaurante), 1, archivo);
    ...
    actualizarUbicacionRestauranteXid(idBuscado);
...
void actualizarUbicacionRestauranteXid(int id_ubicacion){
    ...
    archivo = fopen("ubicacion_restaurante.dat", "r+b");

    if(archivo == NULL){
        printf("\nError al abrir ubicacion_restaurante.dat");
        return;
    }

    while(fread(&ubicacion, sizeof(UbicacionRestaurante), 1, archivo) == 1){

        if(ubicacion.id_restaurante == id_ubicacion){
            ...
            fseek(archivo, -sizeof(UbicacionRestaurante), SEEK_CUR);
            fwrite(&ubicacion, sizeof(UbicacionRestaurante), 1, archivo);
...
void agregarPedido(Usuario usuario){
    ...
    agregarOrdenPreparacion(pedido);
    // Crear Detalles de la Orden
    agregarDetalleOrden(pedido, seleccion, 1);
...
//Agregar Orden en base al pedido
...
void agregarDetalleOrden(Pedido pedido, int id_producto, int cantidad){
    ...
    if(archivo == NULL){
        printf("\nError al abrir detalle_orden.dat");
        return;
    }
    ...
    fwrite(&detalle, sizeof(DetalleOrden), 1, archivo);

    fclose(archivo);
}

//Encargado - Actualiza el pedido
void actualizarPedido(Usuario usuario){
    ...
    scanf("%d", &idPedido);
    if(esCliente(usuario.id_usuario)){
        actualizarPedidoXid(idPedido, "Cancelado");
    }
    else if(esEncargado(usuario.id_usuario)){
        ...
        switch(opcion){
            ...
            case 2:{
                actualizarPedidoXid(idPedido, "Preparado");
                break;
            }
            case 3:{
                actualizarPedidoXid(idPedido, "En_Camino");
                break;
            }
            case 4:{
                actualizarPedidoXid(idPedido, "Listo");
                break;
            }
            default:{
                printf("\nOpcion invalida.");
...
void actualizarPedidoXid(int id_pedido, char nuevoEstado[]){
    ...
    if(encontrado){
        remove("pedido.dat");
        rename("auxiliar.dat", "pedido.dat");
...
void listarHistorialEstados(){
    ...
    while(fread(&estado, sizeof(HistorialEstado), 1, archivo) == 1){
        printf("\nPedido #%d", estado.id_pedido);
        printf("\nEstado: %s", estado.estado);
        printf("\nFecha: %s", estado.fecha_actualizacion);
        printf("\nHora : %s\n", estado.hora_actualizacion);
    }
    fclose(archivo);
}
void listarOrdenesPreparacion(){
    ...
    printf("\n===============================\n");
    printf("\n   ORDENES DE PREPARACION\n");
    printf("\n===============================\n");

    archivo1 = fopen("orden_preparacion.dat", "rb");
    archivo2 = fopen("detalle_orden.dat", "rb");

    if(archivo1 == NULL || archivo2 == NULL){
        printf("\nNo tienes ordenes de preparacion.\n");
        return;
    }

    while(fread(&orden, sizeof(OrdenPreparacion), 1, archivo1) == 1){
        if(strcmp(orden.estado_preparacion, "Pendiente")  == 0){
            printf("\nOrden #%d", orden.id_orden);
            printf("\n================================");
            rewind(archivo2);

            while(fread(&detalle, sizeof(DetalleOrden), 1, archivo2) == 1){
                if(detalle.id_orden == orden.id_orden){
                    printf("\n  Producto: %d", detalle.id_producto);
                    listarProductoPorId(detalle.id_producto);
...
void aceptarPedido(Usuario usuario){
    ...
    actualizarPedidoXid(id_buscar, "En_Camino");
}
// =========================
