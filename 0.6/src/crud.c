    ...
    while(fread(&pedido, sizeof(Pedido), 1, archivo) == 1){
        ...
        if(pedido.id_repartidor == id_usuario &&
           strcmp(pedido.estado, "En_Camino") == 0){
            ...
            char pedidoCamino[250];
            sprintf(pedidoCamino, "ID Pedido: %d | Dirección: %s | Estado: %s", pedido.id_pedido, pedido.direccion_entrega, pedido.estado);
            mostrarInfo(pedidoCamino);
            char obsMsg[220];
            sprintf(obsMsg, "Observaciones: %s", pedido.observaciones);
            mostrarInfo(obsMsg);
            mostrarLinea();
        }
    }
...
void calificarPedido(Usuario usuario){
    ...
    mostrarInfo("Tus pedidos entregados:");
    while(fread(&pedido, sizeof(Pedido), 1, archivoPedidos) == 1){
        if(pedido.id_cliente == usuario.id_usuario && strcmp(pedido.estado, "Entregado") == 0){
            char pedidoInfo[200];
            sprintf(pedidoInfo, "Fecha: %s | Estado: %s", pedido.fecha_pedido, pedido.estado);
