...
void actualizarEstadoOrden(int id_restaurante){
    ...
    while(fread(&orden, sizeof(OrdenPreparacion), 1, archivo) == 1){
        if(orden.id_orden == id_orden_buscar){
            encontrado = 1;
            printf("\n✓ Orden encontrada.\n");
            printf("  ID: %d | Estado Actual: %s\n", orden.id_orden, orden.estado_preparacion);
            printf("\nSeleccione nuevo estado:\n");
            printf("[1] Pendiente\n");
            printf("[2] Confirmado\n");
            printf("[3] Preparandose\n");
            printf("[4] Listo\n");
            printf("Opción: ");
            ...
            switch(opcion_estado){
                case 1:
                    ...
                    break;
                case 2:
                    strcpy(orden.estado_preparacion, "Confirmado");
                    break;
                case 3:
                    strcpy(orden.estado_preparacion, "Preparandose");
                    break;
                case 4:
                    strcpy(orden.estado_preparacion, "Listo");
                    break;
                default:
                    printf("\n[ERROR] Opción inválida.\n");
                    opcion_estado = -1;
            ...
            if(opcion_estado != -1){
                ...
                HistorialEstado historial;
                FILE *archivoHistorial = fopen("historial_estado.dat", "ab");
                if(archivoHistorial != NULL){
                    historial.id_historial = orden.id_pedido;
                    historial.id_pedido = orden.id_pedido;
                    strcpy(historial.estado, orden.estado_preparacion);
                    time_t ahora = time(NULL);
                    struct tm *info = localtime(&ahora);
                    strftime(historial.fecha_actualizacion, 20, "%Y-%m-%d", info);
...
void seleccionarCuentaConRol(Usuario usuario, int opcion_app){
    ...
    int encontrado = 0;
    int tiene_rol = 0;
    int rol_requerido = opcion_app - 1;  // 1=Cliente(0), 2=Repartidor(2), 3=Encargado(1), 4=Admin(3)

    // Mapeo correcto de opción a rol
    if(opcion_app == 1) rol_requerido = 0;      // Cliente
    else if(opcion_app == 2) rol_requerido = 2; // Repartidor
    else if(opcion_app == 3) rol_requerido = 1; // Encargado
    else if(opcion_app == 4) rol_requerido = 3; // Admin
    ...
    if(!roles_actuales[rol_requerido]){
        printf("\nNo tienes permiso para acceder a esta app.\n");
        printf("Tu cuenta no tiene el rol de ");
        switch(opcion_app){
            case 1: printf("Cliente"); break;
            case 2: printf("Repartidor"); break;
            case 3: printf("Encargado"); break;
            case 4: printf("Administrador"); break;
        }
        printf(".\n");
        return;
    }
    ...
    switch(opcion_app){
        case 1: panelCliente(usuario); break;
        case 2: panelRepartidor(usuario); break;
        case 3: panelEncargador(usuario); break;
        case 4: panelAdministrador(usuario); break;
    }
}
...
void registrarUsuarioConRol(int opcion_app){
    ...
    FILE *archivoUsuario, *auxUsuario;
    int ultimoId = 0;

    archivoUsuario = fopen("usuario.dat", "rb");
    if(archivoUsuario != NULL){
        Usuario auxUsr;
        while(fread(&auxUsr, sizeof(Usuario), 1, archivoUsuario) == 1){
            if(auxUsr.id_usuario > ultimoId){
                ultimoId = auxUsr.id_usuario;
            }
        }
        fclose(archivoUsuario);
    }
