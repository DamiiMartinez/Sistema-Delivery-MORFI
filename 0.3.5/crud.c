...
void agregarPedido(Usuario usuario){
    ...
    // LOOP: Seleccionar múltiples productos
    int seguir = 1;
    while(seguir){
        system("cls");
        printf("\n===============================\n");
...
void listarOrdenesRestaurante(int id_restaurante){
    ...
    while(fread(&orden, sizeof(OrdenPreparacion), 1, archivoOrdenes) == 1){
        // Buscar el pedido correspondiente para validar que sea del restaurante
        fseek(archivoPedidos, 0, SEEK_SET);
        
        while(fread(&pedido, sizeof(Pedido), 1, archivoPedidos) == 1){
            if(pedido.id_pedido == orden.id_pedido){
                // Aquí verificaríamos que sea del restaurante correcto
                // Por ahora mostramos todas las órdenes de preparación activas
    ...
    while(fread(&orden, sizeof(OrdenPreparacion), 1, archivo) == 1){
        if(orden.id_orden == id_orden_buscar){
            ...
            if(opcion_estado != -1){
                ...
                // Registrar cambio en historial también
                HistorialEstado historial;
...
void calcularComisionPedido(int id_pedido, int id_pago, int id_restaurante, double monto_total){
    ...
    comision.porcentaje_comision = 15.0;  // 15% comisión del sistema
...
void verFinanzasTotales(){
    ...
    // Calcular comisiones totales (ingresos del sistema)
    archComisiones = fopen("comision.dat", "rb");
    if(archComisiones != NULL){
        while(fread(&comision, sizeof(ComisionDetalle), 1, archComisiones) == 1){
            ingresos_sistema += comision.monto_comision;
            pedidos_totales++;
        }
        fclose(archComisiones);
    }

    // Calcular montos pagados a restaurantes
    archPagos = fopen("pago_restaurante.dat", "rb");
    if(archPagos != NULL){
...
void calificarPedido(Usuario usuario){
    ...
    // Obtener próximo ID de calificación
    ...
    calificacion.id_restaurante = pedido.id_restaurante;  // Esperar, no tenemos esto en Pedido
